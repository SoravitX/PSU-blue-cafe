...
// แสดงเลขออเดอร์ใหม่: หัวการ์ด/คัดลอก/โมดัล = YYMMDD-### และ badge = ### ต่อวัน
...
/* ===== Timezone (ไทย) ให้ PHP/MySQL ตรงกัน ===== */
...
/** คืน YYMMDD-### จาก order_date (Y-m-d) + order_seq */
function format_order_code(string $order_date, int $order_seq): string {
  $d = date_create_from_format('Y-m-d', $order_date);
  if (!$d) { $d = new DateTime($order_date); }
...
/* --------- First visit: default = วันนี้ 00:00–23:59 --------- */
$is_first_visit =
  !isset($_GET['status']) &&
  !isset($_GET['q']) &&
...
if ($date_from==='' && $date_to==='' && $time_from==='' && $time_to==='') {
  $active_qf = 'all';
} elseif ($date_from===$todayYmd && $date_to===$todayYmd && $tf==='00:00' && $tt==='23:59') {
  $active_qf = 'today';
} elseif ($date_from===$sevenStart && $date_to===$todayYmd && $tf==='00:00' && $tt==='23:59') {
  $active_qf = '7d';
}

/* --------- Query orders (ดึง order_date, order_seq ด้วยเสมอ) --------- */
...
<div class="wrap">
  ...
  <?php
  // Map order_id -> label ที่จะใช้ในโมดัล (YYMMDD-### หรือ #id)
  $orderLabelMap = [];
  foreach ($orders as $o) {
    $label = '';
    if (!empty($o['order_date']) && !empty($o['order_seq'])) {
      $label = format_order_code((string)$o['order_date'], (int)$o['order_seq']);
    }
    $orderLabelMap[(int)$o['order_id']] = $label !== '' ? $label : ('#'.(int)$o['order_id']);
  }
  ?>
  ...
  <?php if(!empty($orders)): ?>
    <div class="grid" id="orderGrid">
      <?php foreach($orders as $o):
        ...
        // === สร้างเลขออเดอร์แสดงผล ===
        $order_seq_fmt = !empty($o['order_seq']) ? format_order_seq((int)$o['order_seq']) : '';
        $order_code    = (!empty($o['order_date']) && !empty($o['order_seq']))
                          ? format_order_code((string)$o['order_date'], (int)$o['order_seq'])
                          : '';
        $badgeLabel    = $order_seq_fmt !== '' ? $order_seq_fmt : ('#'.$oid);    // ป้าย = ### หรือ #ID
        $headLabel     = $order_code !== '' ? $order_code : ('#'.$oid);          // หัวการ์ด/คัดลอก = YYMMDD-### หรือ #ID
      ?>
      <div class="card-order" data-status="<?= h($o['status']) ?>">
        ...
        <!-- ป้ายแสดง ### ต่อวัน -->
        <div class="id-badge"><?= h($badgeLabel) ?></div>

        <div class="co-head">
          <div>
            <div class="oid">
              <?= h($headLabel) ?>
              <i class="bi bi-clipboard-plus copy" title="คัดลอกเลขออเดอร์" data-copy="<?= h($headLabel) ?>"></i>
            </div>
            <div class="meta">
              <i class="bi bi-clock-history"></i> <?= h($o['order_time']) ?>
              • <i class="bi bi-basket2"></i> <?= $itemCount ?> รายการ
              • <i class="bi bi-hash"></i> ID <?= $oid ?>
            </div>
          </div>
          <div class="badges">
...
(function(){
  ...
  // Map order_id -> label (YYMMDD-### หรือ #id) ใช้ในหัวโมดัล
  const orderLabelMap = <?php
    echo json_encode($orderLabelMap, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
  ?>;
  ...
  function openModal(oid){
    ...
    mdlOid.textContent = orderLabelMap[key] || ('#' + oid);
    listBox.innerHTML = '';
    if (!items.length) {
  ...
  document.addEventListener('click', (e)=>{
    ...
    // คัดลอกเลขออเดอร์ (YYMMDD-### หรือ #ID)
  ...
  // ===== Quick filters =====
// ===== Quick filters (ใช้ Local Time แทน UTC) =====
document.querySelectorAll('[data-qf]').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    // คืน YYYY-MM-DD ตามเขตเวลาเครื่องผู้ใช้
    const toLocalYMD = (d)=>{
    ...
    if (type==='today') {
      ...
      set('date_from', d); set('time_from','00:00');
      set('date_to',   d); set('time_to','23:59');
    } else if (type==='7d') {
      const end   = toLocalYMD(today);
      // นับย้อนหลัง 6 วัน (รวมวันนี้ = 7 วัน)
      const startDate = new Date(today.getFullYear(), today.getMonth(), today.getDate() - 6);
      const start = toLocalYMD(startDate);
      set('date_from', start); set('time_from','00:00');
      set('date_to',   end  ); set('time_to','23:59');
    } else {
      set('date_from',''); set('time_from','');
      set('date_to','');   set('time_to','');
    }
...
<script>
/* ===== สีข้อความชิปให้แมตช์หน้า front_store ===== */
(function(){
